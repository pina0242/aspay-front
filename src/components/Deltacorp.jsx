...
export const Deltacorp = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const { token, userToDelete } = location.state || {}; // El 'userToDelete' es el objeto 'corporation'
...
          Error: Token o datos de la corporación no encontrados.
        </div>
      </div>
    );
  }
...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/deltacorp`, {
        method: 'POST',
        headers: {
...
      } else {
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar llave corporativa';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {
...
            <p>
              ¿Está seguro de que desea eliminar la corporación con ID **{userToDelete.id}** y datos **{userToDelete.datos}**?
            </p>

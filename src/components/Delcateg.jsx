...
export const Delcateg = () => {
    ...
    return (
      <div className="main-container">
        <div className="error-message">
          Error: Token o datos del nivel de transacción no encontrados.
        </div>
      </div>
    );
  }
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/delcateg`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar el nivel de transacción.';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              ¿Está seguro de que desea eliminar la categoria**{userToEdit.categoria}** con ID **{userToEdit.id}** y nombre **{userToEdit.nombre}**?
            </p>

...
export const Deldir = () => {
  ...
  const handleDelete = () => {
    ...
    const payload = {
      num_id: userToEdit.num_id,
      tipo_dir: userToEdit.tipo_dir,
    };
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/deldir`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar la dirección';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              ¿Está seguro de que desea eliminar la direccion **{userToEdit.direccion}** con ID **{userToEdit.num_id}** con Tipo de Dir **{userToEdit.tipo_dir}**?
            </p>

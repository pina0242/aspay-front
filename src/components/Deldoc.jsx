...
export const Deldoc = () => {
  ...
  const handleDelete = () => {
    ...
    const payload = {
      ...
      tipo_docto: userToEdit.tipo_docto,
    };
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/deldoc`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar el documento.';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              ¿Está seguro de que desea eliminar documento **{userToEdit.tipo_docto}** del ID **{userToEdit.num_id}** ?
            </p>

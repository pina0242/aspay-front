...
export const Listmon = () => {
  ...
  const handleSubmit = (e) => {
    ...
    // Mensaje a encriptar con fechas capturadas o valores por defecto
    const payload = {
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/listmon`, {
        method: 'POST',
        headers: {
      ...
      // Manejo de error 401: Sesión caducada
      ...
      // Manejo de error 400: Recuperar valor sin decriptar
      if (response.status === 400) {
        const errorData = await response.json().catch(() => null);
        const errorMessage = errorData && errorData[0]?.response ? errorData[0].response : 'Error en la solicitud.';
        setError(errorMessage);
        setLoading(false);
        return;
      }

      // Éxito 201: Se procede a desencriptar la respuesta
      if (response.status === 201) {
        const responseData = await response.json();
        setDecryptingResponse(responseData);
      } else {
        setError('Error inesperado al obtener la lista de monitoreo.');
        setLoading(false);
      }
  ...
  return (
    <div className="main-container">
      ...
      <div className="listusr-content-container">
        <h1 className="listusr-title">Listado Monitoreo</h1>
        ...
        {monList.length > 0 && (
          <div className="table-container">
            <h2 className="grid-title">Registros Encontrados</h2>

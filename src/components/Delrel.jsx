...
export const Delrel = () => {
  ...
  const handleDelete = () => {
    ...
    const payload = {
      num_id_princ: userToEdit.num_id_princ,
      num_id_relac: userToEdit.num_id_relac,
      tipo_relac: userToEdit.tipo_relac,
    };
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/delrel`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar la relacion.';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              ¿Está seguro de que desea eliminar la relacion  **{userToEdit.num_id_relac}** con ID Principal **{userToEdit.num_id_princ}** con Tipo de Rel **{userToEdit.tipo_relac}**?
            </p>

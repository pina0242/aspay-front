...
export const Listrel = () => {
  ...
  const [perRel, setPerRel] = useState([]);
  ...
  const handleEditClick = (item) => {
    navigate('/Updrel', { state: { token, userToEdit: item } });
  };
  
  // Función para manejar la eliminación
  const handleDeleteClick = (item) => {
    // Redirige al componente Delrel y pasa los datos del item
    navigate('/Delrel', { state: { token, userToEdit: item } });
  };
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/selrel`, {
        method: 'POST',
        headers: {
      ...
      if (response.status !== 201) {
        const errorData = await response.json().catch(() => null);
        const errorMessage = errorData && errorData[0]?.response ? errorData[0].response : 'Error al obtener Relacion.';
        setError(errorMessage);
        setLoading(false);
        return;
      }
  ...
  return (
    <div className="main-container">
      <div className="depth-2-frame-0">
        ...
        <div className="depth-3-frame-1">
          <div className="depth-4-frame-02">
            ...
            <div className="depth-5-frame-02" style={{ cursor: 'pointer' }} onClick={() => navigate('/Regrel', { state: { token } })}>
              <div className="product">Registra Relaciones</div>
            </div>
      ...
      <div className="listusr-content-container">
        <h1 className="listusr-title">Lista Relaciones</h1>
        <form className="listusr-form" onSubmit={handleSubmit}>
          <div className="form-group">
            <label htmlFor="startDate">ID Principal</label>
            <input type="number" id="numidprinc" value={numidprinc} onChange={(e) => setNumidprinc(e.target.value)} />
          </div>
         
          <button type="submit" disabled={loading}>
            {loading ? 'Cargando...' : 'Consultar Relaciones'}
          </button>
        </form>
      ...
      {perRel.length > 0 && (
        <div className="table-container">
          <h2 className="grid-title">Consulta Relaciones</h2>
          <table className="user-table">
            <thead>
              <tr>
                <th>ID</th>
                <th>ID Principal</th>
                <th>Numero ID Relacion</th>
                <th>Tipo de Relacion</th>
                <th>Nivel de Relacion</th>
                <th>Porcentaje de Participacion</th>
                <th>Fecha Inicio de Relacion</th> 
                <th>Fecha Fin de Relacion</th> 
            ...
            <tbody>
              {perRel.map((item, index) => (
                <tr key={index}>
                  <td>{item.id}</td>
                  <td>{item.num_id_princ}</td>
                  <td>{item.num_id_relac}</td>
                  <td>{item.tipo_relac}</td>
                  <td>{item.nivel_relac}</td>
                  <td>{item.porcentaje_partic}</td>
                  <td>{item.fecha_ini_rel}</td>
                  <td>{item.fecha_fin_rel}</td>
                  <td>{item.docto_referencia}</td>

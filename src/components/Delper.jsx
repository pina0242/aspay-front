...
export const Delper = () => {
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/delper`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar la persona.';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              ¿Está seguro de que desea eliminar la persona **{userToEdit.nombre}** con ID **{userToEdit.num_id}** y IDP **{userToEdit.id_persona}**?
            </p>

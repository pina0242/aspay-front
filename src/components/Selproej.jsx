...
export const Selproej = () => {
    ...
    // Tomamos los datos iniciales del estado de la ruta
    const { token, encryptedData: initialEncryptedData, entidad, filename, ingestId } = location.state || {}; 

    const [processData, setProcessData] = useState(null);
    const [currentEncryptedData, setCurrentEncryptedData] = useState(initialEncryptedData); // Nuevo estado para los datos encriptados
    ...
    // 1. Lógica de inicio/re-descifrado: Se ejecuta cuando llegan nuevos datos encriptados.
    useEffect(() => {
        if (!token || !ingestId) {
        ...
        // Si hay datos encriptados (ya sea iniciales o de re-consulta), activamos el descifrado.
        if (currentEncryptedData) {
            setDecrypting(true);
            setLoading(true); 
            setError(''); // Limpiar errores al iniciar el proceso
        } else {
            // Esto solo debería ocurrir si initialEncryptedData era nulo/undefined
            if (initialEncryptedData === undefined) {
    ...
    }, [token, ingestId, currentEncryptedData, initialEncryptedData]); // Dependencia clave: currentEncryptedData
    ...
    const handleConsultarClick = () => {
        ...
        // Usamos el total actual para la nueva consulta (mapeado a 'inserted')
    ...
    const handleEncryptedData = async (encryptedBody) => {
        setEncrypting(false);
        setDataToProcess(null); // Limpiar el payload de encriptación
        ...
        try {
            ...
            if (response.status === 201 || response.status === 200) {
                ...
                // **CORRECCIÓN CLAVE:** Actualizar el estado local que alimenta al Decryptor
                setCurrentEncryptedData(responseData); 
                // El useEffect se encargará de activar setDecrypting(true)
                
            } else {
                const errorData = await response.json().catch(() => ({ response: 'Error de formato' }));
        ...
        <div className="main-container">
            ...
            <div className="listusr-content-container">
                ...
                {loading && <div className="loading-message">
                    {decrypting ? 'Descifrando respuesta...' : encrypting ? 'Preparando consulta...' : 'Consultando...'}
                </div>}
                ...
                {processData && (
                    <div className="progress-card">
                        ...
                        >
                            {loading && !encrypting && !decrypting ? 'Consultando...' : 'Consultar Avance'}
                        </button>
                    </div>
                )}
            </div>
            ...
            {(decrypting) && currentEncryptedData && (
                <Decryptor
                    encryptedMessage={currentEncryptedData} // Usa el estado local (CORRECCIÓN)

...
export const Delcosto = () => {
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/delcosto`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar costo';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {
      ...
      {showConfirm && (
        <div className="modal-overlay">
          <div className="modal-content">
            ...
            <p>
              ¿Está seguro de que desea eliminar costo **{userToEdit.indcosto}** con ID **{userToEdit.id}**?
            </p>

...
import '../styles/delusr.css'; // Reutilizar estilos para el contenedor
...
export const Delusr = () => {
  ...
  const { token, userToDelete } = location.state || {};
    ...
    return (
      <div className="main-container">
        <div className="error-message">
          Error: Token o datos de usuario no encontrados.
        </div>
      </div>
    );
  }
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/delusr`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorData = responseData[0]?.response ? responseData[0].response : 'Error al eliminar el usuario.';
        setError(errorData);
        setLoading(false);
      }
    } catch (err) {

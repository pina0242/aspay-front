...
export const Regdatcom = () => {
  ...
  const handleEncryptedData = async (encryptedBody) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/regdcomper`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 201) {
        ...
        const errorMessage = responseData && responseData[0]?.response ? responseData[0].response : 'Error al registrar datos complementarios.';
        setError(errorMessage);
        setLoading(false);
      }
    } catch (err) {
  ...
  return (
    <div className="main-container">
      ...
      <div className="regusr-content-container">
        <h1 className="regusr-title">Registrar Datos Complementarios</h1>
        ...
        <div className="form-group">
          <div className="form-grid">
            ...
            <input type="email" id="emailp" value={emailp} onChange={(e) => setEmailp(e.target.value)} maxLength="50" required />
            ...
            <label htmlFor="nivel">Indicador PEP</label>
            ...
            <input type="number" id="volumentx" value={volumentx} onChange={(e) => setVolumentx(e.target.value)} maxLength="10" required/>     
          ...
          <button type="submit" disabled={loading}>
            {loading ? 'Cargando...' : 'Registrar Datos Complementarios'}
          </button>
        </form>
